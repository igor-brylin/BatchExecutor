...
namespace BatchExecutor
{
            ...
            _batchExecutor(argumentsSegment).ContinueWith(t =>
                                                          {
                                                              ...
                                                              var results = faulted ? null : t.Result;
                                                              for (var i = 0; i < bufferLength; i++)
                                                              {
                                                                  var workItem = buffer[i];
                                                                  if (faulted)
                                                                  {
                                                                      workItem.Callback(t.Exception.UnwrapAggregateException(), default);
                                                                  }
                                                                  else if (results != null)
                                                                  {
                                                                      if (!results.TryGetValue(workItem.DataItem, out var result))
                                                                      {
                                                                          workItem.Callback(new KeyNotFoundException($"Record for {workItem.DataItem} not found."), default);
                                                                      }
                                                                      else
                                                                      {
                                                                  ...
                                                                  {
                                                                      workItem.Callback(null, default);
                                                                  }
                                                                  else
                                                                  {
                                                                      workItem.Callback(
                                                                          new InvalidOperationException(
                                                                              "Func<IReadOnlyList<TItem>, Task<IDictionary<TItem, TResult>>> batchExecutor must return a Dictionary, but returns null."),
                                                                          default);
                                                                  }
                                                              }
    ...
    public class BatchExecutor<TItem> : IBatchExecutor<TItem>
    {
        private readonly BatchExecutor<TItem, bool> _base;

        public BatchExecutor(int batchSize, Func<IReadOnlyList<TItem>, Task> batchExecutor, TimeSpan bufferFlushInterval)
        {
            _base = new BatchExecutor<TItem, bool>(batchSize, async items =>
                                                              {
                                                                  await batchExecutor(items).ConfigureAwait(false);
                                                                  return null;
                                                              }, bufferFlushInterval) { AllowNullResult = true };
        }


        public Task ExecAsync(TItem item)
        {
            return _base.ExecAsync(item);
        }
